    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            if ($validator->errors()->isEmpty()) {
                ...
                // Get current entry ID from route parameter
                // Normalize route parameter to ID (handles both model binding and direct ID)
                $param = $this->route('timetable_entry') ?? $this->route('entry');
                $currentEntryId = is_object($param) ? $param->id : $param;

                // 1. Check for class conflict (excluding current entry)
                ...
                // 2. Check for teacher conflict (excluding current entry)
                if ($teacherId) {
