...
<div class="container mx-auto px-4 py-6">
    <div class="flex flex-col md:flex-row items-center justify-between mb-6">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('fees.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg inline-flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                </svg>
                Add New Fee
            </a>
            <a href="{{ route('fees.reports') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-lg inline-flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <x-stat-card 
            title="Total Fees" 
            value="{{ number_format($statistics['totalFees'] ?? 0, 2) }} BDT" 
        ...
        <x-stat-card 
            ...
            value="{{ number_format($statistics['collectedFees'] ?? 0, 2) }} BDT" 
            icon='<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>'
            color="green"
            change="{{ number_format($statistics['collectionRate'] ?? 0, 1) }}%"
            changeText="collection rate"
        />
        
        <x-stat-card 
            title="Pending Fees" 
            value="{{ number_format($statistics['pendingFees'] ?? 0, 2) }} BDT" 
            icon='<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>'
            color="yellow"
            change="{{ ($statistics['unpaidCount'] ?? 0) + ($statistics['partialCount'] ?? 0) }}"
            changeText="pending invoices"
        />
        
        <x-stat-card 
            title="Overdue Fees" 
            value="{{ number_format($statistics['overdueFees'] ?? 0, 2) }} BDT" 
            icon='<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>'
            color="red"
            change="{{ $fees->filter(fn($f) => $f->due_date < now() && $f->status !== 'paid')->count() }}"
