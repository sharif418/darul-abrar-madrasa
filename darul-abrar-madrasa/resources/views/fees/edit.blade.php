...
<div class="container mx-auto px-4 py-6">
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex space-x-2">
            <x-button href="{{ route('fees.show', $fee->id) }}" color="secondary">
        ...
        <form action="{{ route('fees.update', $fee->id) }}" method="POST" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <x-input id="amount" type="number" name="amount" value="{{ old('amount', $fee->amount) }}" step="0.01" min="0" class="block mt-1 w-full" required />
                    <x-input-error for="amount" class="mt-2" />
                </div>
                
                <div>
                    <x-label for="due_date" value="Due Date" />
                    <x-input id="due_date" type="date" name="due_date" value="{{ old('due_date', $fee->due_date->format('Y-m-d')) }}" class="block mt-1 w-full" required />
                ...
                <div id="payment_details" class="{{ in_array(old('status', $fee->status), ['paid', 'partial']) ? '' : 'hidden' }}">
                    ...
                    <x-input id="paid_amount" type="number" name="paid_amount" value="{{ old('paid_amount', $fee->paid_amount) }}" step="0.01" min="0" class="block mt-1 w-full" />
                ...
                <div id="transaction_id_div" class="{{ in_array(old('status', $fee->status), ['paid', 'partial']) ? '' : 'hidden' }}">
                    ...
                    <x-input id="transaction_id" type="text" name="transaction_id" value="{{ old('transaction_id', $fee->transaction_id) }}" class="block mt-1 w-full" />
                    <x-input-error for="transaction_id" class="mt-2" />
                </div>
                
                <div class="md:col-span-2">
                    <x-label for="remarks" value="Remarks" />
                    <x-textarea id="remarks" name="remarks" class="block mt-1 w-full" rows="3">{{ old('remarks', $fee->remarks) }}</x-textarea>
