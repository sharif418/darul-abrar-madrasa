...
@section('header', 'Edit Lesson Plan')

@section('content')
<div class="container mx-auto px-4 py-6">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Edit Lesson Plan</h1>
        <div class="flex space-x-2">
            <x-button href="{{ route('lesson-plans.show', $lessonPlan->id) }}" color="secondary">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="p-6 bg-gray-50 border-b border-gray-200">
            ...
            <p class="text-gray-600 mt-1">Update the lesson plan details.</p>
        </div>

        <form action="{{ route('lesson-plans.update', $lessonPlan->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <x-input id="plan_date" type="date" name="plan_date" value="{{ old('plan_date', $lessonPlan->plan_date->format('Y-m-d')) }}" class="block mt-1 w-full" required />
                    <x-input-error for="plan_date" class="mt-2" />
                </div>

                <div class="md:col-span-2">
                    <x-label for="title" value="Lesson Title" />
                    <x-input id="title" type="text" name="title" value="{{ old('title', $lessonPlan->title) }}" class="block mt-1 w-full" required />
                    <x-input-error for="title" class="mt-2" />
                </div>

                <div class="md:col-span-2">
                    <x-label for="description" value="Lesson Description" />
                    <x-textarea id="description" name="description" class="block mt-1 w-full" rows="6" required>{{ old('description', $lessonPlan->description) }}</x-textarea>
                ...
                <div>
                    ...
                    <x-select id="status" name="status" class="block mt-1 w-full" required>
                        <option value="pending" {{ old('status', $lessonPlan->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                        <option value="completed" {{ old('status', $lessonPlan->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                    </x-select>
                    <x-input-error for="status" class="mt-2" />
                </div>

                <div id="completion_notes_container" class="{{ old('status', $lessonPlan->status) == 'completed' ? '' : 'hidden' }}">
                    <x-label for="completion_notes" value="Completion Notes" />
                    <x-textarea id="completion_notes" name="completion_notes" class="block mt-1 w-full" rows="3">{{ old('completion_notes', $lessonPlan->completion_notes) }}</x-textarea>
