...
<div class="container mx-auto px-4 py-8">
    ...
    <nav class="text-sm mb-4">
        <ol class="list-none p-0 inline-flex">
            ...
            <li class="text-gray-500">Add Entry</li>
        </ol>
    </nav>

    <!-- Header -->
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Add Timetable Entry</h1>
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('timetables.entries.store', $timetable) }}" method="POST" id="entryForm">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="room_number" id="room_number" value="{{ old('room_number') }}" placeholder="e.g., Room 101" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('room_number')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" id="notes" rows="2" placeholder="Any special instructions or notes" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <div class="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <ul class="text-sm text-blue-700 space-y-1">
            <li>• Select day first to see available periods for that day</li>
            <li>• Select class to see subjects for that class</li>
            <li>• The system will automatically detect conflicts (teacher/room double-booking)</li>
